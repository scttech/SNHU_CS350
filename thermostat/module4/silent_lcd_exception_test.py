...
#    2          Add an exception to prevent a silent exception
#------------------------------------------------------------------
...
## Display the script name and PID
script_name = os.path.basename(sys.argv[0])
print(f"{script_name} running — PID: {os.getpid()}  (use: sudo kill -SIGUSR1 {os.getpid()})")
...
while repeat:
    try:
        ...
        ##
        ## To test this exception code
        ## In a second terminal, find the PID first:
        ## sudo pkill -SIGUSR1 -f silent_lcd_exception_test.py
